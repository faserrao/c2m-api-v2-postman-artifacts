...
ACH bank transfer payment variant.
...
| Field / Variant | Type | Required | Description |
| --- | --- | --- | --- |
| `amount` | number | Required | Monetary amount as a numeric value. |
| `currency` | enum (USD | EUR | GBP | CAD | AUD) | Required | ISO 4217 currency code (USD, EUR, GBP, CAD, or AUD). |
...
| Field / Variant | Type | Required | Description |
| --- | --- | --- | --- |
| `cardType` | enum (visa | mastercard | discover | americanExpress) | Required | Card brand enum: visa, mastercard, discover, or americanExpress. |
...
| `year` | integer | Required | Expiration year as a four-digit integer (e.g. 2029). |
...
| Field / Variant | Type | Required | Description |
| --- | --- | --- | --- |
| `documentClass` | enum (letter | postcard | brochure | flat) | Required | Document class determining the mailpiece format. |
| `layout` | enum (address_on_first_page | address_on_back_page) | Required | Page layout controlling where the recipient address is printed. |
| `productionTime` | enum (next_day | two_day | three_day | standard | same_day) | Required | Production time preference (next_day, two_day, etc.). |
| `envelope` | enum (standard | none | flat | double_window) | Required | Envelope type used for the mailing. |
| `color` | enum (full_color | black_and_white) | Required | Color mode for printing (full_color or black_and_white). |
| `paperType` | enum (white | white_24 | ivory | glossy) | Required | Paper stock for printing. |
| `printOption` | enum (double_sided | single_sided) | Required | Duplex setting (double_sided or single_sided). |
| `mailClass` | enum (first_class | standard | non_profit) | Required | USPS mail class (first_class, standard, or non_profit). |
...
| Field / Variant | Type | Required | Description |
| --- | --- | --- | --- |
| `creditCardPayment` | object | — | Credit card payment variant. |
| `invoicePayment` | object | — | Invoice payment — billed to an existing account invoice. |
| `achPayment` | object | — | ACH bank transfer payment variant. |
...
Card brand enum: visa, mastercard, discover, or americanExpress.
...
Color mode for printing (full_color or black_and_white).
...
ISO 4217 currency code (USD, EUR, GBP, CAD, or AUD).
...
Document class determining the mailpiece format.
...
Envelope type used for the mailing.
...
Page layout controlling where the recipient address is printed.
...
USPS mail class (first_class, standard, or non_profit).
...
Duplex setting (double_sided or single_sided).
...
Production time preference (next_day, two_day, etc.).
...
Ordered list of document references to merge into one output document (POST /mail-merge).
...
Expiration year as a four-digit integer (e.g. 2029).
