...
// Submit a multi doc merge job
RecommendedAPI.submitMultiDocMergeParams(submitMultiDocMergeParams: submitMultiDocMergeParams) { (response, error) in
...
// Submit a multi doc job
RecommendedAPI.submitMultiDocParams(submitMultiDocParams: submitMultiDocParams) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// Submit a single doc job
RecommendedAPI.submitSingleDocParams(submitSingleDocParams: submitSingleDocParams) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
