...
func main() {
	...
	resp, r, err := apiClient.MostCommonAPI.SubmitMultiDocMergeParams(context.Background()).SubmitMultiDocMergeParams(submitMultiDocMergeParams).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `MostCommonAPI.SubmitMultiDocMergeParams``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `SubmitMultiDocMergeParams`: StandardResponse
	fmt.Fprintf(os.Stdout, "Response from `MostCommonAPI.SubmitMultiDocMergeParams`: %v\n", resp)
}
```
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.MostCommonAPI.SubmitMultiDocParams(context.Background()).SubmitMultiDocParams(submitMultiDocParams).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `MostCommonAPI.SubmitMultiDocParams``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `SubmitMultiDocParams`: StandardResponse
	fmt.Fprintf(os.Stdout, "Response from `MostCommonAPI.SubmitMultiDocParams`: %v\n", resp)
}
```
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.MostCommonAPI.SubmitSingleDocParams(context.Background()).SubmitSingleDocParams(submitSingleDocParams).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `MostCommonAPI.SubmitSingleDocParams``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `SubmitSingleDocParams`: StandardResponse
	fmt.Fprintf(os.Stdout, "Response from `MostCommonAPI.SubmitSingleDocParams`: %v\n", resp)
}
```
