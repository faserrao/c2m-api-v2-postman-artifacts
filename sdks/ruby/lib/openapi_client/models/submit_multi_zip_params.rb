  ...
  class SubmitMultiZipParams
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::SubmitMultiZipParams` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::SubmitMultiZipParams`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'multi_zip_jobs')
        if (value = attributes[:'multi_zip_jobs']).is_a?(Array)
          self.multi_zip_jobs = value
        end
      else
        self.multi_zip_jobs = nil
      end
    ...
    # @param [Object] multi_zip_jobs Value to be assigned
    def multi_zip_jobs=(multi_zip_jobs)
      if multi_zip_jobs.nil?
        fail ArgumentError, 'multi_zip_jobs cannot be nil'
      end

      @multi_zip_jobs = multi_zip_jobs
    end
