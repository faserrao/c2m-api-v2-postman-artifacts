  ...
  class MultiDocJobItem
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::MultiDocJobItem` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::MultiDocJobItem`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'doc_source_all')
        self.doc_source_all = attributes[:'doc_source_all']
      else
        self.doc_source_all = nil
      end
    ...
    def list_invalid_properties
      ...
      if @doc_source_all.nil?
        invalid_properties.push('invalid value for "doc_source_all", doc_source_all cannot be nil.')
      end
    ...
    # @param [Object] doc_source_all Value to be assigned
    def doc_source_all=(doc_source_all)
      if doc_source_all.nil?
        fail ArgumentError, 'doc_source_all cannot be nil'
      end

      @doc_source_all = doc_source_all
    end
    ...
    def hash
      [job_template, doc_source_all, recipient_address_source].hash
    end
