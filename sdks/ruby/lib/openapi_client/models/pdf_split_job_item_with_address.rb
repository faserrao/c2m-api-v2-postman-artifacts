  ...
  class PdfSplitJobItemWithAddress
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::PdfSplitJobItemWithAddress` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::PdfSplitJobItemWithAddress`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'end_page')
        self.end_page = attributes[:'end_page']
      else
        self.end_page = nil
      end
    ...
    # @param [Object] start_page Value to be assigned
    ...
    # Custom attribute writer method with validation
    # @param [Object] end_page Value to be assigned
    ...
    def hash
      [job_template, start_page, end_page, recipient_address_source].hash
    end
