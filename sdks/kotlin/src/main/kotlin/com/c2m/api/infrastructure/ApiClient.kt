package com.c2m.api.infrastructure
...
open class ApiClient(val baseUrl: String, val client: Call.Factory = defaultClient) {
    companion object {
        ...
        const val baseUrlKey: String = "com.c2m.api.baseUrl"
    ...
    protected fun <T> updateAuthParams(requestConfig: RequestConfig<T>) {
        ...
        if (requestConfig.headers["X-Client-Id"].isNullOrEmpty()) {
            if (apiKey["X-Client-Id"] != null) {
                if (apiKeyPrefix["X-Client-Id"] != null) {
                    requestConfig.headers["X-Client-Id"] = apiKeyPrefix["X-Client-Id"]!! + " " + apiKey["X-Client-Id"]!!
                } else {
                    requestConfig.headers["X-Client-Id"] = apiKey["X-Client-Id"]!!
                }
            }
        }
    }
